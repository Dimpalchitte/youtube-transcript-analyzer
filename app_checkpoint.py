...
def get_transcript(url, target_lang='en'):
    try:
        ...
        # Fetch transcript and properly handle the data structure
        transcript_pieces = []
        for piece in transcript.fetch():
            if isinstance(piece, dict) and 'text' in piece:
                transcript_pieces.append(piece['text'])
            elif hasattr(piece, 'text'):
                transcript_pieces.append(piece.text)
                
        full_text = ' '.join(transcript_pieces)
            ...
            try:
                with transcript_container:
                    with st.spinner("Fetching transcript..."):
                        transcript_data = get_transcript(url, target_language)
